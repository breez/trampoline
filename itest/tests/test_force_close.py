...
def test_force_close(node_factory):
    ...
    wait_for(
        lambda: sender.rpc.listpeerchannels(peer_id=trampoline.info["id"])["channels"][
            0
        ]["state"]
        == "AWAITING_UNILATERAL"
    )
    ...
    wait_for(
        lambda: trampoline.rpc.listpeerchannels(peer_id=sender.info["id"])["channels"][
            0
        ]["state"]
        == "ONCHAIN"
    )
    ...
    rawtx, txid, blocks = trampoline.wait_for_onchaind_tx(
        "THEIR_HTLC_FULFILL_TO_US", "THEIR_UNILATERAL/THEIR_HTLC"
    )
    assert blocks == 0
